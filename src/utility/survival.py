...
class survival:
    ...
    def predict_survival_funciton(self, model, X_test, y_test) -> pd.DataFrame:
        lower, upper = np.percentile(y_test[y_test.dtype.names[1]], [10, 90])
        times = np.arange(lower, upper+1, dtype=int)
        surv_prob = np.row_stack([fn(times) for fn in model.predict_survival_function(X_test)])
        return pd.DataFrame(surv_prob, columns=times)

    def predict_hazard_function(self, model, X_test, y_test) -> pd.DataFrame:
        lower, upper = np.percentile(y_test[y_test.dtype.names[1]], [10, 90])
        times = np.arange(lower, upper+1, dtype=int)
        ...
        def score_diff(X_1, X_2, y_1, y_2, estimator1_best_fts, estimator2_best_fts):
            if estimator1.__class__.__name__ == "XGBRegressor":  # Manually handle XGB case
                ...
                if any(np.isnan(preds)):
                    ...
                    est1_score = concordance_index_censored(y_2['Observed'], y_2['Weeks'], preds)[0]
            elif estimator1.__class__.__name__ == "type": # Manually handle WeibullAFT case
                wf = estimator1()
                estimator1_best_fts = np.append(estimator1_best_fts, 'Observed')
                X_1_wf = pd.concat([X_1.reset_index(drop=True),
                                    pd.DataFrame(y_1['Observed'], columns=['Observed'])], axis=1)
                ...
                try:
                    ...
                    X_2_wf = pd.concat([X_2.reset_index(drop=True),
                                        pd.DataFrame(y_2['Observed'], columns=['Observed'])], axis=1)
                    preds = wf.predict(X_2_wf.loc[:, estimator1_best_fts])
                    if any(np.isnan(preds)):
                        est1_score = 0
                    else:
                        est1_score = concordance_index(y_2['Weeks'], preds, y_2['Observed'])
                except:
                    est1_score = 0
            else:
            ...
            if estimator2.__class__.__name__ == "XGBRegressor":
                ...
                if any(np.isnan(preds)):
                    ...
                    est2_score = concordance_index_censored(y_2['Observed'], y_2['Weeks'], preds)[0]
            elif estimator2.__class__.__name__ == "type":
                wf = estimator2()
                estimator2_best_fts = np.append(estimator2_best_fts, 'Observed')
                X_1_wf = pd.concat([X_1.reset_index(drop=True),
                            pd.DataFrame(y_1['Observed'], columns=['Observed'])], axis=1)
                y_1_wf = np.array([x[1] for x in y_1], float)
                try:
                    ...
                    if any(np.isnan(preds)):
                        ...
                        est2_score = concordance_index(y_2['Weeks'], preds, y_2['Observed'])
                except:
                    est2_score = 0
            else:
